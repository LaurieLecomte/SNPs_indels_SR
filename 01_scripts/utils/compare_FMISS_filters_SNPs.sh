...
bcftools view -m2 -M2 --threads $CPU $RAW_SNP | bcftools +setGT --threads $CPU -- -t q -n . -e "FORMAT/DP >= $MIN_DP & FORMAT/DP < $MAX_DP & FORMAT/GQ >= $MIN_GQ & FORMAT/GQ < $MAX_GQ" | bcftools +fill-tags --threads $CPU -- -t AC,AC_Hom,AC_Het,AC_Hemi,AF,AN,NS,ExcHet,HWE,MAF,F_MISSING | bcftools sort | grep -vFf $REGIONS_EX > $FILT_DIR/"$(basename -s .vcf.gz $RAW_SNP)"_DP"$MIN_DP"_GQ"$MIN_GQ"_tagged.vcf
bgzip $FILT_DIR/"$(basename -s .vcf.gz $RAW_SNP)"_DP"$MIN_DP"_GQ"$MIN_GQ"_tagged.vcf -f
...
do 
  bcftools view --max-alleles 2 $FILT_DIR/"$(basename -s .vcf.gz $RAW_SNP)"_DP"$MIN_DP"_GQ"$MIN_GQ"_tagged.vcf.gz | bcftools filter -i "INFO/MAF >= $MIN_MAF & INFO/MAF <= $MAX_MAF & INFO/F_MISSING <= $MAX_MISS" | bcftools sort -Oz > $FILT_DIR/"$(basename -s .vcf.gz $RAW_SNP)"_MAF"$MIN_MAF"_FMISS"$MAX_MISS".vcf.gz
